...
[forex/README.md](./forex/README.md) tách Forex thành learning path riêng nhưng vẫn nằm dưới `05_trading_derivatives/`. Path hiện đi từ cấu trúc OTC và instrument, quote/P&L, leverage–margin–position sizing, relative macro/rates/carry tới execution, strategy research, portfolio/microstructure/options, Korea–Vietnam market context, rồi institutional FX với NDF, forward points, cross-currency basis, funding, intervention, reserves, REER và valuation. Xem thêm [Forex Coverage Audit](./forex/COVERAGE_AUDIT.md) để kiểm tra depth gate và boundary; không mở rộng theo kiểu indicator/candlestick encyclopedia.
...
Với Forex, cần tách thêm `spot direction` khỏi `forward/funding economics`: NDF, basis, collateral, hedge roll và intervention/regime có thể thay đổi economics dù chart spot nhìn giống nhau.
