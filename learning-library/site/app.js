...
const state = { docs: [], query: '', filter: 'all' };
...
function renderCards() {
  ...
  const docs = state.docs.filter(doc => (state.filter === 'all' || doc.category === state.filter) && (!term || `${titleOf(doc)} ${doc.path} ${doc.category}`.toLowerCase().includes(term)));
  document.querySelector('#result-count').textContent = `${docs.length}개 자료`;
  document.querySelector('#document-grid').innerHTML = docs.length ? docs.map(doc => `<a class="doc-card" href="#/read/${encodeURIComponent(doc.path)}"><div class="doc-meta"><span class="type-badge">${doc.type}</span><span>${formatSize(doc.size)}</span></div><h3>${escapeHtml(titleOf(doc))}</h3><p class="doc-path">${escapeHtml(doc.category)} / ${escapeHtml(doc.path)}</p></a>`).join('') : '<p class="empty">검색 조건에 맞는 자료가 없습니다.</p>';
}
...
async function renderReader(path) {
  ...
  if (doc.type === 'PDF') { content.innerHTML = `<iframe class="pdf-frame" title="${escapeHtml(titleOf(doc))}" src="${fileUrl(doc.path)}#view=FitH"></iframe><a class="open-file" href="${fileUrl(doc.path)}" target="_blank" rel="noreferrer">새 탭에서 PDF 열기 ↗</a>`; return; }
  try { const response = await fetch(fileUrl(doc.path)); if (!response.ok) throw new Error(); const markdown = await response.text(); content.className = 'markdown'; content.innerHTML = markdownToHtml(markdown); const headings = [...content.querySelectorAll('h1,h2,h3')]; document.querySelector('#toc').innerHTML = headings.map(h => `<a href="#${h.id}">${escapeHtml(h.textContent)}</a>`).join(''); } catch { content.innerHTML = '<p class="empty">문서를 불러오지 못했습니다.</p>'; }
}
