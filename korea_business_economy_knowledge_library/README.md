...
```mermaid
graph TD
    H0[Pre-1945 initial conditions] --> H1[1945-61 reconstruction]
    H1 --> H2[1960s export industrialization]
    H2 --> H3[1970s HCI / chaebol]
    H3 --> H4[1980s stabilization / liberalization]
    H4 --> H5[1990s globalization / 1997 crisis]
    H5 --> H6[2000s restructuring / broadband / global brands]
    H6 --> H7[2010s-2020s advanced industry / platforms / resilience]
    H7 --> HG[Company genealogies]

    HG --> A[Current Korean economic model]
    A --> M[Macro economy]
    A --> T[Trade / GVC]
    A --> INST[Institutions / policy]

    M --> HH[Households / demographics]
    HH --> PROD[Productivity / dualism]
    PROD --> RD[R&D / human capital]

    T --> CF[Company forms]
    CF --> CH[Chaebol / groups]
    CF --> SME[SME / mid-sized]
    CF --> START[Startup / venture]

    CH --> GS[Group structure]
    GS --> GOV[Governance]
    ...
    IND --> GEO[Industrial geography]
    INST --> PUB[Public enterprises]

    INST --> ENERGY[Energy]
    RD --> BIO[Biohealth]
    RD --> DEF[Defense / aerospace]
    T --> LOG[Logistics]
    START --> DIGI[Digital / fintech / cloud / IT]
```
