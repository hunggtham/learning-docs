   ...
   **/
	...
	*/ 
	function commSelectBox(selectboxObj,parms,firstOption,selectCode){
		$.ajax({
	    	  type			: "POST"
	   		 ,url			: "/Common.do?cmd=commSelectBox"
...
//    	$form.submit();
	...
	*/ 
	...
	function win_course(i_cour){
		$.ajax({
		     type			: "POST"
			, url			: "/home/Train.do?cmd=selectRecentCour"
			, dataType		: "json"
			, data 			: {  i_cour : i_cour }
			, async 		: false
			, contentType: "application/x-www-form-urlencoded; charset=UTF-8",
		success: function(data) {
			...
			if(i_tran == "025001"){
				location.href ="/home/Train.do?cmd=openPage&pageName=trainCourseCollectDetail&i_tran="+i_tran+"&i_cour="+i_cour+"&d_opn_yy="+d_opn_yy+"&o_sim_num="+o_sim_num;	
			}else{
				location.href ="/home/Train.do?cmd=openPage&pageName=trainCourseDetail&i_tran="+i_tran+"&i_cour="+i_cour+"&d_opn_yy="+d_opn_yy+"&o_sim_num="+o_sim_num;	
			}
		},
		error: function(jqXHR, textStatus, errorThrown) {
		  // display an error message in any way
	...
	function win_course_mobile(i_cour){
		...
		success: function(data) {
			...
			if(i_tran == "025001"){
				location.href ="/home/Train.do?cmd=openPage&pageName=trainCourseCollectDetail&mobile_yn=Y&i_tran="+i_tran+"&i_cour="+i_cour+"&d_opn_yy="+d_opn_yy+"&o_sim_num="+o_sim_num;	
			}else{
				location.href ="/home/Train.do?cmd=openPage&pageName=trainCourseDetail&mobile_yn=Y&i_tran="+i_tran+"&i_cour="+i_cour+"&d_opn_yy="+d_opn_yy+"&o_sim_num="+o_sim_num;	
			}
		},
		error: function(jqXHR, textStatus, errorThrown) {
		  // display an error message in any way
	...
	function popup_open() {
		...
		if(popupId == "popup_menge")
			location.href ="/home/Train.do?cmd=openPage&pageName=trainCollectMain";
		if(popupId == "popup_remote")
			location.href ="/home/Train.do?cmd=openPage&pageName=trainReomteMain";
		if(popupId == "popup_qual")
			location.href ="/home/Qual.do?cmd=openPage&pageName=qualMain";
		if(popupId == "popup_book")
			location.href ="/home/Book.do?cmd=openPage&pageName=bookMain";
	}
	...
	function popup_close() {
		...
		qAni();
		$("#"+popupId).hide();
		popupValue = popup_getcookie(popupId);
 		$("#quick_top").hide();	
	...
	function checkCertHist(d_opn_yy, i_cour, o_sim_num, o_tk_lect, o_reg) {
	    ...
	    jQuery.ajax({
	        type          : "POST"
	      , url           : "/home/MyKbi.do?cmd=selectCertHist"
	...
	function checkAllowInfo(d_opn_yy, i_cour, o_sim_num, o_tk_lect) {
	    ...
	    jQuery.ajax({
	        type          : "POST"
	      , url           : "/home/MyKbi.do?cmd=selectAllowInfo"
	...
	function checkTkLectReg(d_opn_yy, i_cour, o_sim_num, o_tk_lect, o_reg) {
	    ...
	    jQuery.ajax({
	        type          : "POST"
	      , url           : "/home/MyKbi.do?cmd=selectTkLectReg"
	      , data          : {"p_d_opn_yy" : d_opn_yy, "p_i_cour" : i_cour, "p_o_sim_num" : o_sim_num, "p_o_tk_lect" : o_tk_lect, "p_reg" : o_reg} 
	      , dataType      : "json"
	      , async         : false
	      , cache         : false
	...
	function getExamTempYn(d_opn_yy, i_cour, o_sim_num, item_cd, test_no, o_tk_lect) {
	    ...
	    jQuery.ajax({
	        type          : "POST"
	      , url           : "/home/MyKbi.do?cmd=selectExamTempYn"
	...
	function checkTkLectBeforeEnd(d_opn_yy, i_cour, o_sim_num, o_tk_lect) {
	    ...
	    jQuery.ajax({
	        type          : "POST"
	      , url           : "/home/MyKbi.do?cmd=selectTkLectBeforeEnd"
	      , data          : {"p_d_opn_yy" : d_opn_yy, "p_i_cour" : i_cour, "p_o_sim_num" : o_sim_num, "p_o_tk_lect" : o_tk_lect} 
	      , dataType      : "json"
	      , async         : false
	      , cache         : false
	        	 ...
	        	 if(result=="N"){
	        		 if(type.split('_').length == 1 && confirm("세션이 만료되었습니다.\n로그인페이지로 이동하시겠습니까?")){
	        			 if(type =='h'){ // 홈페이지
		        			...
		        			url="/platformWeb/Login.do?cmd=moveLogin&mobile_yn=Y&goLogin=Y";
		        			document.location = url;
